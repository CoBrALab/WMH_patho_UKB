...
dir.create("./results/parcnofill_UKB_disorders", showWarnings=FALSE)
dir.create("./visualization/parcnofill_UKB_disorders", showWarnings=FALSE)
...
fwrite(results, "./results/parcnofill_UKB_disorders/WMH_diff_UKB_icd10.tsv", row.names=FALSE, col.names=TRUE, quote=FALSE, sep="\t")
...
fwrite(as.data.frame(avg_beta_region), "./results/parcnofill_UKB_disorders/avg_beta_region.tsv", row.names=FALSE, col.names=TRUE, quote=FALSE, sep="\t")

# Load results
metrics = c("WMHvol", "stage", "MD", "ISOVF", "FA", "ICVF", "OD", "T2star", "QSM")

results = as.data.frame(fread("./results/parcnofill_UKB_disorders/WMH_diff_UKB_icd10.tsv"))
...
for (dx_i in 1:length(levels(results$categ))) {
    ...
    name_file = paste0("./visualization/parcnofill_UKB_disorders/",levels(results$categ)[dx_i], ".png")
