...
dir.create("./results/parcnofill_UKB_PRS", showWarnings=FALSE)
dir.create("./visualization/parcnofill_UKB_PRS", showWarnings=FALSE)
...
demo = as.data.frame(fread("../../../WMH_micro_spatial/Analyses_nm/clean_wmh_data/results/demo.tsv"))

# wmh = as.data.frame(fread("../3_temporal_sustain/results/3f_clean_wmh_data/wmh_combined.tsv"))
wmh = as.data.frame(fread("../../../WMH_micro_spatial/Analyses_nm/clean_wmh_data/results/spect_clust_k3_nofillvox/wmh_combined.tsv"))
...
# save 
fwrite(prs, "./results/parcnofill_UKB_PRS/prs.tsv", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")

# Load data

prs = as.data.frame(fread("./results/parcnofill_UKB_PRS/prs.tsv"))
...
results_prs = results_prs %>%
        filter(PRS %in% c("alzheimer_s_disease", "cardiovascular_disease", "ischaemic_stroke")) %>%
...
fwrite(results_prs, "./results/parcnofill_UKB_PRS/WMH_diff_UKB_PRS.tsv", row.names=FALSE, col.names=TRUE, quote=FALSE, sep="\t")
...
fwrite(as.data.frame(avg_beta_region), "./results/parcnofill_UKB_PRS/avg_beta_region.tsv", row.names=FALSE, col.names=TRUE, quote=FALSE, sep="\t")

# Load results
results_prs = as.data.frame(fread("./results/parcnofill_UKB_PRS/WMH_diff_UKB_PRS.tsv"))
...
viz_on_brain = function(df, out_name) {
    ...
    mnc_parc = mincGetVolume("../2_spatial_clust/results/2d_assign_excluded_voxels/final_parc_k3.mnc")
        ...
        png(file=paste0("./visualization/parcnofill_UKB_PRS/",out_name,"_",levels(df$PRS)[i],".png"), width=7500, height=1000, pointsize = 150)
