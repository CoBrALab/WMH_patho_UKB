...
def fit_repeated_cv_old(X, y, n_repeats=100, cv=5):
    """
    ...
    print("\n" + "="*60)
    print(f"REPEATED CROSS-VALIDATION ({n_repeats} x {cv}-Fold Stratified)")
    print("="*60)
    ...
    for repeat in range(n_repeats):
        ...
        for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), 1):
            ...
            repeat_metrics['accuracy'].append(accuracy_score(y_test_fold, y_pred))
            repeat_metrics['precision'].append(precision_score(y_test_fold, y_pred, zero_division=0))
            repeat_metrics['recall'].append(recall_score(y_test_fold, y_pred, zero_division=0))
...
def fit_repeated_cv(X, y, n_repeats=100, cv=5, model_type='lasso'):
    """
    Perform repeated stratified k-fold cross-validation
    ...
    first_repeat_y_pred_proba = []
    print("\n" + "="*60)
    model_name = "LASSO LOGISTIC REGRESSION" if model_type == 'lasso' else "RANDOM FOREST"
    ...
    for repeat in range(n_repeats):
        ...
        for fold, (train_idx, test_idx) in enumerate(skf.split(X, y), 1):
            ...
            if model_type == 'lasso':
                ...
                # Random Forest doesn't need scaling, use original data
                model.fit(X_train_fold, y_train_fold)
                X_test_scaled = X_test_fold  # Use unscaled for RF
            elif model_type == 'tabfn':  # TabFN (https://doi.org/10.1038/s41586-024-08328-6)
    ...
    print(f"SUMMARY ACROSS {n_repeats} REPEATS")
    print("-"*60)
    for metric in ['balanced_accuracy', 'accuracy', 'precision', 'recall', 'f1', 'roc_auc']:
        scores = all_repeat_metrics[metric]
        print(f"{metric.upper()}:")
        print(f"  Mean: {np.mean(scores):.4f}")
        print(f"  Std:  {np.std(scores):.4f}")
...
df_metrics.to_csv("./results/spect_clust_k3_micro_metrics.csv", index=False)

model_clust_rf, y_true, y_pred, y_pred_proba, fold_metrics = fit_repeated_cv(X_clust, y_clust, cv=5, model_type='rf')
df_metrics = pd.DataFrame(fold_metrics)
df_metrics.to_csv("./results/spect_clust_k3_micro_metrics_rf.csv", index=False)

# Volumes only
model_clust_vol, y_true, y_pred, y_pred_proba, fold_metrics = fit_repeated_cv(X_clust_vol, y_clust, cv=5, model_type='lasso')
df_metrics = pd.DataFrame(fold_metrics)
df_metrics.to_csv("./results/spect_clust_k3_vol_metrics.csv", index=False)

model_clust_vol_rf, y_true, y_pred, y_pred_proba, fold_metrics = fit_repeated_cv(X_clust_vol, y_clust, cv=5, model_type='rf')
df_metrics = pd.DataFrame(fold_metrics)
df_metrics.to_csv("./results/spect_clust_k3_vol_metrics_rf.csv", index=False)
...
df_metrics = pd.DataFrame(fold_metrics)
df_metrics.to_csv("./results/pv_deep_micro_metrics.csv", index=False)

model_pvdeep_rf, y_true, y_pred, y_pred_proba, fold_metrics = fit_repeated_cv(X_pvdeep, y_pvdeep, cv=5, model_type='rf')
df_metrics = pd.DataFrame(fold_metrics)
df_metrics.to_csv("./results/pv_deep_micro_metrics_rf.csv", index=False)

# Volumes only
model_pvdeep_vol, y_true, y_pred, y_pred_proba, fold_metrics = fit_repeated_cv(X_pvdeep_vol, y_pvdeep, cv=5, model_type='lasso')
df_metrics = pd.DataFrame(fold_metrics)
df_metrics.to_csv("./results/pv_deep_vol_metrics.csv", index=False)

model_pvdeep_vol_rf, y_true, y_pred, y_pred_proba, fold_metrics = fit_repeated_cv(X_pvdeep_vol, y_pvdeep, cv=5, model_type='rf')
df_metrics = pd.DataFrame(fold_metrics)
df_metrics.to_csv("./results/pv_deep_vol_metrics_rf.csv", index=False)
...
df_metrics = pd.DataFrame(fold_metrics)
df_metrics.to_csv("./results/noparc_micro_metrics.csv", index=False)

model_noparc_rf, y_true, y_pred, y_pred_proba, fold_metrics = fit_repeated_cv(X_noparc, y_noparc, cv=5, model_type='rf')
df_metrics = pd.DataFrame(fold_metrics)
df_metrics.to_csv("./results/noparc_micro_metrics_rf.csv", index=False)

# Volumes only
model_noparc_vol, y_true, y_pred, y_pred_proba, fold_metrics = fit_repeated_cv(X_noparc_vol.reshape(-1,1), y_noparc, cv=5)
df_metrics = pd.DataFrame(fold_metrics)
df_metrics.to_csv("./results/noparc_vol_metrics.csv", index=False)

model_noparc_vol_rf, y_true, y_pred, y_pred_proba, fold_metrics = fit_repeated_cv(X_noparc_vol.reshape(-1,1), y_noparc, cv=5)
df_metrics = pd.DataFrame(fold_metrics)
df_metrics.to_csv("./results/noparc_vol_metrics_rf.csv", index=False)
...
metrics_clust_rf = pred_predict(model_clust_rf, X_prs, y_prs)
metrics_clust_rf['run'] = "clust_vol_micro"
metrics_clust_vol_rf = pred_predict(model_clust_vol_rf, X_prs_vol, y_prs)
metrics_clust_vol_rf['run'] = "clust_vol"
...
metrics_pvdeep_rf = pred_predict(model_pvdeep_rf, X_prs, y_prs)
metrics_pvdeep_rf['run'] = "pvdeep_vol_micro"
metrics_pvdeep_vol_rf = pred_predict(model_pvdeep_vol_rf, X_prs_vol, y_prs)
metrics_pvdeep_vol_rf['run'] = "pvdeep_vol"
...
metrics_noparc_rf = pred_predict(model_noparc_rf, X_prs, y_prs)
metrics_noparc_rf['run'] = "noparc_vol_micro"
metrics_noparc_vol_rf = pred_predict(model_noparc_vol_rf, X_prs_vol, y_prs)
metrics_noparc_vol_rf['run'] = "noparc_vol"
...
metrics_all.to_csv("results/all_prs_metrics.csv")

metrics_all_rf = pd.concat([metrics_clust_rf, metrics_clust_vol_rf, metrics_pvdeep_rf, metrics_pvdeep_vol_rf, metrics_noparc_rf, metrics_noparc_vol_rf])
metrics_all_rf.to_csv("results/all_prs_metrics_rf.csv")

#endregion

################### OLD

# #region Load data for spect_clust_k3
...
# df_stroke = df_stroke.drop(columns=['ID'])
# df_stroke['Group'] = 'Stroke'
# df_stroke['dataset'] = 'UKB'

# df_dementia_ukb = pd.merge(dementia_cases, df_ukb)
# df_dementia_ukb = df_dementia_ukb.drop(columns=['ID'])
# df_dementia_ukb['Group'] = 'cog_imp'
# df_dementia_ukb['dataset'] = 'UKB'
...
# ]
# # Keep only MCI or dementia
# df_demo = df_demo[df_demo['dx'].isin(['MCI', 'Dementia'])].reset_index(drop=True)
# df_demo = df_demo.loc[:,["ID", "date"]]

# df_ci_adni = pd.merge(df_demo, df_adni, how="left", on=['ID', 'date'])
# df_ci_adni['Group'] = 'cog_imp'
# df_ci_adni['dataset'] = 'ADNI'

# df_ci_adni = df_ci_adni.loc[:,df_stroke.columns]

# # Combine datasets
# df_all = pd.concat([df_stroke, df_dementia_ukb, df_ci_adni], axis=0, ignore_index=True)
# # df_all = pd.concat([df_stroke, df_dementia_ukb], axis=0, ignore_index=True)
# df_all = df_all.dropna().reset_index(drop=True)

# y_clust = df_all['Group'].map({'Stroke': 0, 'cog_imp': 1}).values
# # X = df_all.drop(columns=['Group', 'dataset'])
# # X['sex'] = X['sex'].map({'Male': 1, 'Female': 0})
# X_clust = df_all.drop(columns=['Group', 'dataset', 'age', 'sex'])
# X_clust = X_clust.values

# X_clust_vol = df_all.loc[:,["WMHvol_c1", "WMHvol_c2", "WMHvol_c3"]].values

# #endregion

# #region Load data for PV/deep
...
# y_pv_deep = df_all['Group'].map({'Stroke': 0, 'cog_imp': 1}).values
# # X = df_all.drop(columns=['Group', 'dataset'])
# # X['sex'] = X['sex'].map({'Male': 1, 'Female': 0})
# X_pv_deep = df_all.drop(columns=['Group', 'dataset', 'age', 'sex'])
# X_pv_deep = X_pv_deep.values

# X_pv_deep_vol = df_all.loc[:,["pv_deep_WMHvol_c1", "pv_deep_WMHvol_c2"]].values
