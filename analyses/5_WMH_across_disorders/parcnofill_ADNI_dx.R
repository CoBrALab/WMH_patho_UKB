...
dir.create("./results/parcnofill_ADNI_dx", showWarnings=FALSE)
dir.create("./visualization/parcnofill_ADNI_dx", showWarnings=FALSE)
...
df_wmh = as.data.frame(fread("../../../WMH_micro_spatial/Analyses_nm/clean_wmh_data/results/spect_clust_k3_nofillvox/wmh_combined_ADNI.tsv"))
...
# Write to tsv
fwrite(results, "./results/parcnofill_ADNI_dx/WMH_diff_ADNI_dx.tsv", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")
...
for (var in vars_to_viz) {
    ...
    name_file = paste0("./visualization/parcnofill_ADNI_dx/dx_",var, ".png")
...
# Write to tsv
fwrite(results, "./results/parcnofill_ADNI_dx/WMH_diff_ADNI_abplus.tsv", col.names=TRUE, row.names=FALSE, quote=FALSE, sep="\t")
...
# Visualize results on brain map
name_file = paste0("./visualization/parcnofill_ADNI_dx/abplus_coef.png")
png(file=name_file, width=5100, height=1000, pointsize = 150)
sliceSeries(nrow=1, ncol=1, begin=52, end=52, dimension=3) %>%
    addtitle(names[1]) %>%
    anatomy(anatVol, low=10, high=200) %>%
