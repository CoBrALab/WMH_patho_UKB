...
dir.create("./results/2j_clust_all_voxels_viz", showWarnings=FALSE)
dir.create("./visualization/2j_clust_all_voxels_viz", showWarnings=FALSE)
...
for (cluster_number in clust_list) {
  ...
  dir.create(paste0("./visualization/2j_clust_all_voxels_viz/k",cluster_number), showWarnings=FALSE)

  clust_number = cluster_number
  clust_final = read.csv(paste0("./results/2i_spectral_clust_all_voxels/clust_final_k",clust_number,".csv"))
  ...
  clust_final$cluster <- factor(clust_final$cluster, levels = md_means)
  ...
  mincWriteVolume(outvol, paste0("./results/2i_spectral_clust_all_voxels/k",clust_number,"_all_clusters.mnc"), clobber=TRUE)
  ...
  png(file=paste0("./visualization/2j_clust_all_voxels_viz/k",clust_number,"_all_clusters.png"), width=3000, height=3000, pointsize = 80)
  sliceSeries(nrow=3, ncol=3, begin=40, end=52, dimension=3) %>%
    anatomy(anatVol, low=10, high=140) %>%
    overlay(mincArray(mincGetVolume(paste0("./results/2i_spectral_clust_all_voxels/k",clust_number,"_all_clusters.mnc"))),
  ...
  for (c in 1:clust_number){
    ...
    ggsave(paste0("./visualization/2j_clust_all_voxels_viz/k",clust_number,"_allMicro_cluster",c,".png"),
...
ggplot(anovas_k3, aes(x=micro, y=R2, fill=micro)) + 
  ...
  theme(text=element_text(size=30, color="black"), plot.title=element_text(hjust=0.5, size=40),
  legend.position = "none")
ggsave("./visualization/2j_clust_all_voxels_viz/k3_var_explained.png", width=10, height=7)
