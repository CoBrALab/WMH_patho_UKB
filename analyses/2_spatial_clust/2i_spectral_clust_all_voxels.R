...
dir.create("./results/2i_spectral_clust_all_voxels", showWarnings=FALSE)
dir.create("./visualization/2i_spectral_clust_all_voxels", showWarnings=FALSE)
...
df = fread(paste0("./results/2a_make_averages/WMH_zscore_",maps[1],"_avg_NAWM1000_WMH1.tsv"))

for (m in 2:length(maps)) {
  df = cbind(df, fread(paste0("./results/2a_make_averages/WMH_zscore_",maps[m],"_avg_NAWM1000_WMH1.tsv")))
}
...
write.csv(elbow, "./results/2i_spectral_clust_all_voxels/eigengap.csv", row.names = FALSE)
...
ggsave("./visualization/2i_spectral_clust_all_voxels/nclust_elbow.png", width = 12, height=6)
...
for (k in 1:(range_of_ks-1)){
  ...
  write.csv(clust_final, paste0("./results/2i_spectral_clust_all_voxels/clust_final_k",k+1,".csv"), row.names = FALSE)
}
